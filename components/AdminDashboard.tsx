...
export default function AdminDashboard({ usuario, onLogout }: AdminDashboardProps) {
  ...
  const cargarTodo = async () => {
    ...
    try {
      const [resAeropuertos, resVuelos, resUsuarios, resReservas, resStats] = await Promise.all([
        fetch(`${API_URL}/admin/aeropuertos`),
        fetch(`${API_URL}/admin/vuelos`),
        fetch(`${API_URL}/admin/usuarios`),
        fetch(`${API_URL}/admin/reservas`),
  ...
  const handleSubmitVuelo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit
        ? `${API_URL}/admin/vuelos/${editandoVueloId}`
        : `${API_URL}/admin/vuelos`;
  ...
  const handleEliminarVuelo = async (vueloId: number) => {
    ...
    try {
      const response = await fetch(`${API_URL}/admin/vuelos/${vueloId}`, { method: 'DELETE' });
      const data = await response.json();
      if (!response.ok) {
  ...
  const handleSubmitUsuario = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit
        ? `${API_URL}/admin/usuarios/${editandoUsuarioId}`
  ...
  const handleEliminarUsuario = async (usuarioId: number) => {
    ...
    try {
      const response = await fetch(`${API_URL}/admin/usuarios/${usuarioId}`, { method: 'DELETE' });
      const data = await response.json();
      if (!response.ok) {
